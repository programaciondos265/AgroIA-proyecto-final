...
export function HistoryModal({ onClose }: HistoryModalProps) {
  ...
  const loadHistory = useCallback(async () => {
    try {
      ...
      setIsLoading(false);
      console.log('🏁 HistoryModal - Loading finalizado');
    }
  }, []);

  useEffect(() => {
    console.log('🔄 HistoryModal - useEffect ejecutado');
    loadHistory();
  }, [loadHistory]);

  // Log cuando el estado cambia
  useEffect(() => {
    console.log('🔄 HistoryModal - Estado history cambió:', history.length, 'elementos');
  }, [history]);

  useEffect(() => {
    console.log('🔄 HistoryModal - Estado isLoading cambió:', isLoading);
  }, [isLoading]);

  useEffect(() => {
    console.log('🔄 HistoryModal - Estado error cambió:', error);
  }, [error]);
  ...
  const deleteAnalysis = async () => {
    if (itemToDelete) {
      try {
        ...
        await loadHistory(); // Recargar el historial después de eliminar
      } catch (error) {
  ...
  // Log del estado actual para depuración (siempre se ejecuta)
  console.log('🔍 HistoryModal - RENDER - Estado actual:', {
    isLoading,
    error,
          ...
          <Card>
            ...
            <HistoryList>
              {isLoading ? (
                ...
                (() => {
                  console.log('🎨 HistoryModal - Renderizando historial con', history.length, 'elementos');
