...
export function AnalysisModal({ result, onClose, imageData, onSave }: AnalysisModalProps) {
  ...
  const pestName = result.detections.length > 0 ? result.detections[0].pestType : "Sin plagas detectadas";
  const confidence = result.detections.length > 0 ? Math.round(result.detections[0].confidence * 100) : 0;
